...
- `scripts/fetch_kkl.py` downloads the GIF, decodes it in memory, saves per-snapshot
  `_last.jpg` (last frame) + extracted `frames/HHMMSS_NNN.jpg`, rebuilds `daily.gif`
  + `strip.jpg` for that IST day, and updates `docs/data/index.json`.
...
- Per-frame JPGs older than 30 days are pruned from the working tree (keeps `daily.gif`/`strip.jpg`/`*_last.jpg` forever). Tweak `RETENTION_DAYS` in workflow.
...
- Repo size: pruning deletes old files from the working tree, but git history keeps every committed blob. Raw GIFs (~1.7 MB × 8/day ≈ 400 MB/mo) are never committed — only extracted JPGs + daily summaries — so growth is far slower than before. If the clone still gets heavy, squash history, e.g.:
  ```
  ...
  # (keep only *_last.jpg + daily.gif/strip.jpg/frames).
  ```
